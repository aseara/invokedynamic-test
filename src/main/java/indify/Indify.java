...
public class Indify {
    ...
    public void run(String... av) throws IOException {
        ...
        if ("--java".equals(avl.get(0))) {
            avl.remove(0);
            try {
                runApplication(avl.toArray(new String[avl.size()]));
            } catch (Exception ex) {
    ...
    public void runApplication(String... av) throws Exception {
        ...
        av = avl.toArray(new String[avl.size()]);
        ...
        try { main.setAccessible(true); } catch (SecurityException ex) { ex.printStackTrace(); }
    ...
    public void indifyJar(@SuppressWarnings("unused")File f,
                          @SuppressWarnings("unused")Object dest)
            throws IOException {
    ...
    public void indifyTree(File f, File dest) throws IOException {
        ...
        for (File f2 : f.listFiles((dir, name) -> {
    ...
    private class Logic {
        ...
        Instruction findPop(Instruction i) {
            ...
            for (i = i.clone().next(); i != null; i = i.next()) {
                ...
                if (pops == null)  break;
                if (jvm.stackMotion(i.bc))  continue;
                if (pops.indexOf('Q') >= 0) {
    ...
    public class ClassFile extends Outer implements Chunk {
        ...
        void writeTo(File f) throws IOException {
            try (DataOutputStream out = openOutput(f)) {
                writeTo(out);
            }
        }
    ...
    public static class Instruction implements Cloneable {
        ...
        public final Instruction clone() {
            try {
        ...
        public int u2At(int pos) {
            return (u1At(pos)<<8) + u1At(pos+1);
        }
        public int u4At(int pos) {
            return (u2At(pos)<<16) + u2At(pos+2);
        }
